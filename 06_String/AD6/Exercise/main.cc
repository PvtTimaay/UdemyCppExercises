...
int main(int argc, char **argv)
{
    ...
    if (argc < 2)
    {
        data_filepath /= std::filesystem::current_path();
        data_filepath /= "data";
    }
    else
    {
        const auto data_path_str = std::string(argv[1]);
        data_filepath = std::filesystem::path(data_path_str);
    }

    std::filesystem::path ego_filepath = data_filepath;
    ego_filepath /= "ego_data.json";
    std::filesystem::path vehicle_filepath = data_filepath;
